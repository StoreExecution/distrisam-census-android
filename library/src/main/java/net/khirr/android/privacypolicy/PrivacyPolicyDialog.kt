...
class PrivacyPolicyDialog(private val context: AppCompatActivity,
    ...
    private fun loadLayout(): View {
        ...
        if (showTerms) {
            layout.termsOfServiceContainer.visibility = View.VISIBLE
        } else {

            layout.termsOfServiceContainer.visibility = View.GONE

        }



        layout.cbAccept.setEnabled(false);
        layout.acceptButton.setOnClickListener {

            if (layout.cbAccept.isChecked || !showTerms) {
                dismiss()
                onClickListener?.onAccept(true)
                policiesAccepted = true
            } else {

                Toast.makeText(context, "Veuillez accepter les terms de l'accord ", Toast.LENGTH_LONG).show();
            }
        }
        ...
        layout.policiesRecyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                super.onScrollStateChanged(recyclerView, newState)

                if (!recyclerView.canScrollVertically(1)) {
                    layout.cbAccept.setEnabled(true);
