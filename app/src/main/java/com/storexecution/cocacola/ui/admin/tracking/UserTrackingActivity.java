package com.storexecution.cocacola.ui.admin.tracking;
...
import com.storexecution.cocacola.adapter.UserTrackAdapter;
...
import com.storexecution.cocacola.ui.rtm.ActivityChangeDialog;
...
public class UserTrackingActivity extends AppCompatActivity implements Marker.OnMarkerClickListener, LocationListener {
    /**
     * ButterKnife Code
     **/
    ...
    private RealmList<UserTrack> userTracks;
    int currentPosition = 0;
    UserTrack currentUserTrack;
    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            public void onItemClick(View v, int position) {
                if (sectors.get(position).getOpen() == 1) {
                    if (sectors.get(position).getLocalFile() != null && sectors.get(position).getLocalFile().length() > 0) {
                        ...
                        pDialog = new SweetAlertDialog(UserTrackingActivity.this, SweetAlertDialog.PROGRESS_TYPE);
                        pDialog.getProgressHelper().setBarColor(Color.parseColor("#A5DC86"));
                        pDialog.setTitleText("Téléchargement Secteur");
                        pDialog.setCancelable(false);
                        pDialog.show();
                        Toasty.info(UserTrackingActivity.this, "Téléchargement du secteur", 4000).show();
                        RealmList<Sector> toDownload = new RealmList<>();
                        toDownload.add(sectors.get(position));
                        downloadSectors(toDownload, 0);
                    }
                } else {
                    Toasty.warning(UserTrackingActivity.this, "Ce secteur n'est pas encore actif", 3000).show();
                }
            }
        });
        ...
        BingMapTileSource.retrieveBingKey(UserTrackingActivity.this);
        setTileSource();
        ...
        userTrackAdapter = new UserTrackAdapter(this, userTracks, selected, new RecyclerItemClickListener() {
            @Override
            public void onItemClick(View v, int position) {
                onUserTrack(position);

            }
        });
        rvUserTracks.setLayoutManager(new LinearLayoutManager(this));
        rvUserTracks.setAdapter(userTrackAdapter);
    ...
    private void loadMarkers() {
        ...
        userTracks.addAll(realm.where(UserTrack.class).sort("order", Sort.ASCENDING).findAll());
        userTrackAdapter.notifyDataSetChanged();
        for (UserTrack salepoint : userTracks) {
    ...
    public void btActivityChange() {
        ...
        activityChangeDialog.setRtmId(currentUserTrack.getUserId());
    ...
    public boolean onMarkerClick(Marker marker, MapView mapView) {
        ...
        onUserTrack(markers.indexOf(marker));
        return true;
    }

    @OnClick(R.id.btAll)
    public void syncRtmBDD() {
        SweetAlertDialog sweetAlertDialog = new SweetAlertDialog(UserTrackingActivity.this);
        sweetAlertDialog.setTitle("Mise a jour de la BDD");


        new MaterialDialog.Builder(UserTrackingActivity.this)
                ...
                .onPositive(new MaterialDialog.SingleButtonCallback() {
                    @Override
                    public void onClick(@NonNull MaterialDialog dialog, @NonNull DialogAction which) {
                        progressDialog = new ProgressDialog(UserTrackingActivity.this);
                        progressDialog.setMessage("Téléchargement..");
                        progressDialog.setIndeterminate(true);
                        progressDialog.setCancelable(false);
                        progressDialog.show();
                        Intent intent = new Intent(UserTrackingActivity.this, DownloadService.class);
    ...
    public void syncSector() {
        ...
        service.fetchSectors(headers, gson.toJson(filter)).enqueue(new Callback<ArrayList<Sector>>() {
            ...
            public void onFailure(Call<ArrayList<Sector>> call, Throwable t) {
                sweetAlertDialog.dismiss();
                sectorAdapter.notifyDataSetChanged();
                Toasty.error(UserTrackingActivity.this, "Erreur de connexion", 3000).show();
    ...
    public void onUserTrack(int position) {

        currentPosition = position;

        currentUserTrack = userTracks.get(position);
        currentPosition = position;
        ...
        loadDetails(currentUserTrack);
        if (!detailsShowing)
            showDetails();
    }

    private void loadDetails(UserTrack i) {
    ...
    public void ImagePreview() {
        ...
        dialog.setOnShowListener(new DialogInterface.OnShowListener() {
            ...
            public void onShow(DialogInterface d) {
                ...
                //   Photo photo = realm.where(Photo.class).equalTo("TypeID", currentUserTrack.getMobile_id()).and().equalTo("Type", Constants.IMG_POS).findFirst();
//                if (UserTrack.getImage() != null && UserTrack.getImage().length() > 0)
//                    iv.setImageBitmap(Base64Util.Base64ToBitmap(UserTrack.getImage(), 1));
...
//        intent.putExtra("rtmId", UserTrack.get());
...
//    }
    ...
    public void setLoation() {
        ...
        if (gpsActive) {
            ...
            Toasty.error(UserTrackingActivity.this, "Veuillez activer votre GPS", 5000).show();
        }
    }
    ...
    private void gpsLocal() {
        ...
        Toasty.info(UserTrackingActivity.this, "Vous Utiliser le GPS ,cela peut prendre plusieurs minutes").show();
...
//        String selectedFilePath = FilePath.getPath(getActivity(), uri);
        ...
        try {
            ...
            if (file.exists()) {
                ...
                try {
                    ...
                    Toasty.error(UserTrackingActivity.this, "Secteur invalide ", 10000).show();
                    loadUserPosition();
                }
    ...
    private void downloadSectors(RealmList<Sector> sectors, int position) {
        ...
        if (sectors.size() > position) {
            ...
            if (pDialog != null && pDialog.isShowing())
                pDialog.dismiss();
            Toasty.success(UserTrackingActivity.this, "Les secteurs ont été téléchargés", 3000).show();
    ...
    protected void onResume() {
        ...
        if (realm != null && userTracks != null && mvMap != null && userTrackAdapter != null)
            loadMarkers();
    ...
    public void route() {
        ...
        routeToSalepoint(currentUserTrack);
    }

    private void routeToSalepoint(UserTrack i) {
        if (progressDialog != null && progressDialog.isShowing())
            progressDialog.dismiss();
    ...
    public void openMaps(Double lat, Double lng) {
        ...
        if (mapIntent.resolveActivity(UserTrackingActivity.this.getPackageManager()) != null) {
